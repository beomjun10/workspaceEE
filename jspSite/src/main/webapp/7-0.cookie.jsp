	...
	<table width="80%" border="0" cellspacing="0" cellpadding="2"
		...
		<tr>
			<td height="100" class="t1"><br> 웹의 근간이 되는 <font
				...
				이미지를 가져오는데 매우<br> 이상적인 프로토콜이지만, 서버측 응용프로그램(CGI, Servlet JSP,
				ASP, PHP)에서 생성되는 상호 대화적인<br> 내용을 처리하는데는 여러 가지 문제가 있다.<br>
				<br> <font color="#0000FF"> HTTP 프로토콜은 브라우저에서 요청이 있을 때
					마다 새로운 네트워크 연결이 이루어지기 때문에<br> 사용자의 요청에 네트워크 연결이 지속되지 않으므로
					클라이언트의 요청을 서로 구분 할수 없으며,<br> 클라이언트의 상태정보를 저장 할 수 없다.
			</font><br> <br> 상태가 없으면 웹을 이용한 응용프로그램(대표적으로 쇼핑몰 구현)을 개발하기 어려워
				지는데,<br> 이것은 사용자가 쇼핑몰을 방문해서 원하는 상품을 선택할 때마다 이전에 선택한 상품정보는<br>
				잃어버리는 문제가 발생하기 때문이다.<br> <br> <font color="#FF00FF">
					이런한 문제를 해결(상태정보 유지)하기 위해서 쿠키와 세션이 등장하게 되었다.</font> <br> <br></td>
		</tr>
				...
				<table width="100%" align="center" border="1" cellspacing="0"
					...
					<tr bgcolor="#f0fff3">
						<td align="left" class="t1"><br> 쿠키는 클라이언트의 상태 정보를 유지하기
							위한 방법으로 네스케이프에서 제안된 기술로<br> 웹응용프로그램에서 필요에 따라<br> <br>
							<font color="#0000CC"> 1) 클라이언트의 정보를 텍스트 형태로
								클라이언트(웹브라우저)에게 전송하고<font color="#FF0000">(응답메세지의 헤더를 통해)</font>,<br>
								<br> 2) 웹 브라우저는 전송받은 텍스트파일(쿠키)을 메모리에 로딩(저장)하고 있다가, 다시 쿠키를
								전송한 해당<br>&nbsp;&nbsp;&nbsp;사이트에 방문할 경우 웹서버에게 제출하도록 한다.<font
								color="#FF0000">(요청메세지의 헤더를 통해)</font><br> <br> 3)
								웹서버는 전달 받은 쿠키의 값을 읽어 들여 적절한 작업을 수행할 수 있다.<br> <br>
							...
							<br> &nbsp;&nbsp;&nbsp;&nbsp;▶ <b> <font color="#0000CC">name=valu</font>e
